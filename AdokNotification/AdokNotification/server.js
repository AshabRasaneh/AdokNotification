...
(function () {
    ...
    var timeout = setInterval(function () {
        try {
            ...
            const result = conLeague.query(query);
            if (result.length > 0) {
                result.forEach((row) => {
                    var id = row.id;
